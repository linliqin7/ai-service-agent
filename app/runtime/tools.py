...
def knowledge_citations(observations):
    """Return only knowledge records actually returned by the search tool."""
    citations=[];seen=set()
    for observation in observations:
        if observation.get('name')!='search_knowledge':continue
        for citation in observation.get('result',{}).get('citations',[]):
            citation_id=citation.get('id')
            if citation_id and citation_id not in seen:
                citations.append(citation);seen.add(citation_id)
    return citations
...
def render_facts(observations,intent):
    ...
    knowledge=knowledge_citations(observations)
    ...
    if intent=='account': answer=f"模拟账户状态：{d['account_status']}。\n开户进度：{d['onboarding']}。\n实际办理请使用开户券商官方 App。"
    elif intent=='funds': answer=f"模拟资金状态\n可用：{d['available']:,} 元\n可取：{d['withdrawable']:,} 元\n冻结：{d['frozen']:,} 元\n可用不等于可取。本接口未提供冻结明细及银行回执，不能进一步断言具体冻结原因或到账时间；有争议可提交人工核查。"
    elif intent=='risk': answer=f"模拟风险测评为 R{d['risk_level']}，{'当前有效' if d['risk_valid'] else '已失效，需在官方 App 重新如实测评'}。风险等级不代表某产品适合购买，不能据此给出投资推荐。"
    elif intent=='holdings': answer='模拟资产：'+f"{d['asset']:,} 元。\n"+ ('\n'.join(f"{h['name']}：{h['quantity']} 份，模拟估值 {h['valuation']:,} 元" for h in d['holdings']) if d['holdings'] else '当前测试账户无持仓。')+'\n没有连接实时行情，不能据此计算真实收益。'
    else: return None
    return _append_knowledge_evidence(answer,knowledge)

def _append_knowledge_evidence(answer,knowledge):
    if not knowledge:return answer
    evidence='\n'.join(f"- {item['text']}" for item in knowledge if item.get('text'))
    return answer+'\n知识依据：\n'+evidence if evidence else answer
